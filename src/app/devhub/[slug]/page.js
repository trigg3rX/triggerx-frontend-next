...
function DevhubItem() {
  ...
  const headerImageUrl = urlFor(postData.image)?.width(1200).auto('format').url();
    ...
    return (
      <div className="rounded-2xl bg-[#F9FFE1] p-6 my-8 text-black shadow">
        <div className="flex items-center mb-3">
          ...
          <h3 className="font-bold text-lg text-black">{value.title || 'Disclaimer'}</h3>
        </div>
        <p className="text-base text-gray-800 leading-relaxed whitespace-pre-wrap">{value.text}</p>
      </div>
    );
  };
                  ...
                  {postData.headingPairs?.map((pair, index) => (
                    <li key={index} className="py-2 px-2">
                      <a
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          const targetElement = document.getElementById(pair.h2Heading);
                          if (targetElement) {
                            const yOffset = -160;
                            const y =
              ...
              components={{
                ...
                block: {
                  ...
                  h3: ({ children }) => {
                    const text = children?.[0];
                    return (
                      <h3 id={text} className="text-sm sm:text-lg xl:text-xl 2xl:text-2xl mt-4">
                        {text}
                      </h3>
                    );
                  },
                  normal: ({ children }) => (
                    <p className="my-2 text-xs sm:text-sm xl:text-base 2xl:text-lg">{children}</p>
                  ),
                },
              }}
            />
