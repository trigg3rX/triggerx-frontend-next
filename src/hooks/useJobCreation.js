...
export function useJobCreation() {
  ...
  const estimateFee = async (timeframeInSeconds, intervalInSeconds, codeUrls) => {
    try {
  ...
  const handleSubmit = async (stakeRegistryAddress, stakeRegistryABI, jobdetails) => {
    if (!jobType) {
    ...
    try {
      ...
      if (userBalance < estimatedFee) {
        ...
        const tx = await contract.stake(ethers.parseEther(requiredEth.toString()), {
