...
const ProcessModal = ({ isOpen, steps, onClose }) => {
  ...
  ethImage.src = 'https://upload.wikimedia.org/wikipedia/commons/0/05/Ethereum_logo_2014.svg';
  ...
  useEffect(() => {
    ...
    character.forEach((segment) => {
      ctx.drawImage(currentFrame, segment.x * tileSize, segment.y * tileSize, tileSize, tileSize);
    });
  ...
  return (
    ...
    >
      ...
      <p className="text-white text-center mt-2">Use arrow keys and collect tokens</p>
    </Modal>
  );
};
